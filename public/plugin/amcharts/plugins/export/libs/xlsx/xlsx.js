...
function parse_comments_xml(data, opts) {
    ...
    (data.match(/<(?:\w+:)?commentList>([^\u2603]*)<\/(?:\w+:)?commentList>/)||["",""])[1].split(/<\/\w*:?comment>/).forEach(function(x, index) {
        ...
        if (!textMatch || !textMatch[1]) return; // a comment may contain an empty text category.
...
function parse_xlml_xml(d, opts) {
    ...
    while((Rn = xlmlregex.exec(str))) switch(Rn[3]) {
        ...
        default:
            ...
            if(!state[state.length-1][1]) throw 'Unrecognized category: ' + Rn[3] + "|" + state.join("|");
            ...
            if(opts.WTF) throw 'Unrecognized category: ' + Rn[3] + "|" + state.join("|");
    }
    var out = {};
